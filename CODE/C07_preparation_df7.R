...
df_np_group = df_np %>%
  group_by(ID_CLI) %>%
  summarise(AVG_REPURCH_TIME = round(mean(DIFF),0)) %>%
  ungroup() %>%
  as.data.frame()

## compute distribution of days between purchases per client ##

x <- as.data.frame(table(df_np_group$AVG_REPURCH_TIME))

## remove purchases made on the same day ##
...
#!!! NOTE: on average, 90% of the customers buy again within 4 weeks !!!#  
